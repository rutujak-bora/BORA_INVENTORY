...
from schemas import (
    ...
    InwardStockCreate,
    InwardStockResponse,
    InwardStockDetailResponse,
    StockSummaryResponse,
    StockTrackingResponse,
    OutwardStockCreate,
    OutwardStockResponse,
    OutwardStockDetailResponse,
)
from auth import (
    get_password_hash,
    verify_password,
...
async def log_cors_headers(request, call_next):
    ...
    if origin:
        logger.info(
            f"[CORS DEBUG] Request from Origin: {origin} | Method: {method} | Path: {path} | Host: {host}"
        )
...
async def get_categories(current_user: dict = Depends(get_current_active_user)):
    ...
    async def get_distinct_safe(collection, field):
        try:
            ...
            logger.error(
                f"Error fetching distinct {field} from {collection.name}: {str(e)}"
            )
            return []
...
async def create_company(
    company_data: CompanyCreate, current_user: dict = Depends(get_current_active_user)
):
    try:
        ...
        if "E11000 duplicate key error" in str(e):
            raise HTTPException(
                status_code=400, detail="Company with this Name or GSTN already exists"
            )
        raise HTTPException(
            ...
            try:
                ...
                company_dict = {
                    ...
                    **({"GSTNumber": gst_value} if gst_value else {}),
                    "apob": (
                        str(row.get("apob", "")).strip()
                        if pd.notna(row.get("apob", ""))
                        else None
                    ),
                    "address": (
                    ...
                    "contact_details": (
                        str(row.get("contact_details", "")).strip()
                        if pd.notna(row.get("contact_details", ""))
                        else None
                    ),
                    "country": (
...
async def bulk_delete_companies(
    company_ids: dict, current_user: dict = Depends(get_current_active_user)
):
    """
...
async def export_companies(
    format: str = "json", current_user: dict = Depends(get_current_active_user)
):
    """
    ...
    if format == "csv":
        ...
        return {"data": companies, "format": "csv"}
...
async def create_product(
    product_data: ProductCreate, current_user: dict = Depends(get_current_active_user)
):
    try:
        product_dict = {
            "id": str(uuid.uuid4()),
...
async def bulk_upload_products(
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            spec_value = row.get(
                "specification",
                row.get("Specification", row.get("default_rate", row.get("Rate", ""))),
            )
            if pd.notna(spec_value):
                specification = str(
                    spec_value
                )  # Keep as string to support both text and numbers
            else:
                specification = None
            ...
            product_dict = {
                "id": str(uuid.uuid4()),
                "sku_name": str(
                    row.get("sku_name", row.get("SKU", row.get("Name", "")))
                ),
                "category": (
                    str(row.get("category", row.get("Category", "")))
                    if pd.notna(row.get("category", row.get("Category", "")))
                    else None
                ),
                "brand": (
                    str(row.get("brand", row.get("Brand", "")))
                    if pd.notna(row.get("brand", row.get("Brand", "")))
                    else None
                ),
                "hsn_sac": (
                    str(row.get("hsn_sac", row.get("HSN", "")))
                    if pd.notna(row.get("hsn_sac", row.get("HSN", "")))
                    else None
                ),
                "country_of_origin": (
                    str(row.get("country_of_origin", row.get("Country", "")))
                    if pd.notna(row.get("country_of_origin", row.get("Country", "")))
                    else None
                ),
                "color": (
                    str(row.get("color", row.get("Color", "")))
                    if pd.notna(row.get("color", row.get("Color", "")))
                    else None
                ),
                "specification": specification,  # Can be text or number
                "feature": (
                    str(row.get("feature", row.get("Feature", "")))
                    if pd.notna(row.get("feature", row.get("Feature", "")))
                    else None
                ),
                "is_active": True,
                "created_at": datetime.now(timezone.utc).isoformat(),
...
async def delete_product(
    ...
    pi_count = await mongo_db.proforma_invoices.count_documents(
        {"line_items.product_id": product_id, "is_active": True}
    )
    po_count = await mongo_db.purchase_orders.count_documents(
        {"line_items.product_id": product_id, "is_active": True}
    )
    ...
    await mongo_db.products.update_one(
        {"id": product_id}, {"$set": {"is_active": False}}
    )
    ...
    try:
        ...
        for _, row in df.iterrows():
            warehouse_dict = {
                ...
                "address": (
                    str(row.get("address", row.get("Address", "")))
                    if pd.notna(row.get("address", row.get("Address", "")))
                    else None
                ),
                "city": (
                    str(row.get("city", row.get("City", "")))
                    if pd.notna(row.get("city", row.get("City", "")))
                    else None
                ),
                "country": (
                    str(row.get("country", row.get("Country", "")))
                    if pd.notna(row.get("country", row.get("Country", "")))
                    else None
                ),
                "contact_details": (
                    str(row.get("contact_details", row.get("Contact", "")))
                    if pd.notna(row.get("contact_details", row.get("Contact", "")))
                    else None
                ),
                "is_active": True,
                "created_at": datetime.now(timezone.utc).isoformat(),
        ...
        return {
            "message": f"Successfully uploaded {len(warehouses)} warehouses",
...
async def get_warehouse(
    warehouse_id: str, current_user: dict = Depends(get_current_active_user)
):
...
async def update_warehouse(
    ...
    updated_warehouse = await mongo_db.warehouses.find_one(
        {"id": warehouse_id}, {"_id": 0}
    )
...
async def delete_warehouse(
    ...
    await mongo_db.warehouses.update_one(
        {"id": warehouse_id}, {"$set": {"is_active": False}}
    )
...
async def create_bank(
    bank_data: BankCreate, current_user: dict = Depends(get_current_active_user)
):
    try:
        bank_dict = {
            "id": str(uuid.uuid4()),
...
async def delete_bank(
    bank_id: str, current_user: dict = Depends(get_current_active_user)
):
    bank = await mongo_db.banks.find_one({"id": bank_id}, {"_id": 0})
    if not bank:
        raise HTTPException(status_code=404, detail="Bank not found")
...
async def download_warehouses_template():
    ...
    data = {
        ...
        "address": [
            "Plot 101, MIDC Area, Andheri East",
            "Sector 18, Industrial Area, Noida",
        ],
        "city": ["Mumbai", "Delhi"],
        "country": ["India", "India"],
    ...
    return StreamingResponse(
        ...
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={
            "Content-Disposition": "attachment; filename=Warehouses_Template.xlsx"
        },
    )
...
@api_router.post("/pi")
async def create_pi(
    pi_data: dict, current_user: dict = Depends(get_current_active_user)
):
    # Create PI
    pi_dict = {
        "id": str(uuid.uuid4()),
...
async def bulk_upload_pis(
    ...
    try:
        ...
        if "voucher_no" not in df.columns:
            raise HTTPException(
                status_code=400, detail="Missing required column: voucher_no"
            )
        ...
        for voucher_no in df["voucher_no"].unique():
            ...
            pi_dict = {
                ...
                "date": str(
                    first_row.get("date", datetime.now(timezone.utc).isoformat())
                ),
            ...
            for _, row in pi_rows.iterrows():
                line_item = {
                    ...
                    "amount": float(row.get("quantity", 0) or 0)
...
async def get_inward_qty_for_pi(
    pi_id: str, product_sku: str, warehouse_id: str, product_id: str = None
) -> float:
...
async def get_dispatched_qty_for_pi(
    ...
    for outward in all_outwards:
        ...
        if (
            outward.get("dispatch_type") == "dispatch_plan"
            and outward.get("id") in linked_plan_ids
        ):
            continue
...
async def get_pi(
    ...
    if warehouse_id:
        ...
        async for stock in mongo_db.inward_stock.find(
            {"pi_id": pi_id, "warehouse_id": warehouse_id}, {"_id": 0}
        ):
    ...
    logger.info(
        f"Returning PI detail for {pi_id}: {pi.get('voucher_no')} with {len(pi.get('line_items', []))} items"
    )
    return pi
...
async def update_pi(
    pi_id: str, pi_data: dict, current_user: dict = Depends(get_current_active_user)
):
    pi = await mongo_db.proforma_invoices.find_one({"id": pi_id}, {"_id": 0})
    if not pi:
        raise HTTPException(status_code=404, detail="PI not found")
...
async def export_pis(
    pi_ids: list[str], current_user: dict = Depends(get_current_active_user)
):
    from fastapi.responses import StreamingResponse
    from io import BytesIO
...
@api_router.post("/po")
async def create_po(
    po_data: dict, current_user: dict = Depends(get_current_active_user)
):
    ...
    if reference_pi_ids:
        for pi_id in reference_pi_ids:
            pi = await mongo_db.proforma_invoices.find_one(
                {"id": pi_id, "is_active": True}, {"_id": 0}
            )
            if not pi:
                raise HTTPException(
                    status_code=404, detail=f"proforma Invoice {pi_id} not found"
                )
    ...
    po_dict = {
        ...
        "reference_pi_id": (
            reference_pi_ids[0] if reference_pi_ids else None
        ...
        "gst_percentage": float(
            po_data.get("gst_percentage", 0)
        ),  # GST % entered manually
        "tds_percentage": float(
            po_data.get("tds_percentage", 0)
        ),  # TDS % entered manually
    ...
    po_dict["total_amount"] = round(
        total_basic_amount + total_gst_value - total_tds_value, 2
    )  # Basic + GST - TDS
...
async def bulk_upload_pos(
    ...
    try:
        ...
        if df.empty:
            ...
            raise HTTPException(
                status_code=400, detail="No valid voucher numbers found in file"
            )
        ...
        for voucher_no in unique_vouchers:
            ...
            if pd.isna(pi_ids_val):
                pi_ids_val = first_row.get("reference_pi_id")

            if pd.notna(pi_ids_val) and str(pi_ids_val).strip():
                pi_ids_str = str(pi_ids_val).strip()
                reference_pi_ids = [
                    pid.strip() for pid in pi_ids_str.split(",") if pid.strip()
                ]
                ...
                po_dict["line_items"].append(
                    {
                        ...
                        "pi_voucher_no": clean_str(
                            row.get(
                                "pi_voucher_no",
                                row.get("pi_no", row.get("pi_number", "")),
                            )
                        ),
                        "quantity": qty,
                        "rate": rate,
        ...
        return {
            "message": f"Successfully uploaded {pos_created} Purchase Orders",
            "count": pos_created,
        }
    ...
    try:
        async for po in mongo_db.purchase_orders.find({"is_active": True}, {"_id": 0}):
            try:
                ...
                logger.error(
                    f"Error processing individual PO {po.get('voucher_no')}: {str(inner_e)}"
                )
                continue
...
async def get_po(
    ...
    logger.info(
        f"Returning PO detail for {po_id}: {response_data.get('voucher_no')} with {len(response_data.get('line_items', []))} items"
    )
...
async def update_po(
    po_id: str, po_data: dict, current_user: dict = Depends(get_current_active_user)
):
    po = await mongo_db.purchase_orders.find_one({"id": po_id}, {"_id": 0})
    if not po:
        raise HTTPException(status_code=404, detail="PO not found")
    ...
    if "line_items" in po_data:
        ...
        update_data["total_amount"] = round(
            total_basic_amount + total_gst_value - total_tds_value, 2
        )
...
async def delete_po(po_id: str, current_user: dict = Depends(get_current_active_user)):
    ...
    await mongo_db.purchase_orders.update_one(
        {"id": po_id}, {"$set": {"is_active": False}}
    )
...
async def export_pos(
    po_ids: list[str], current_user: dict = Depends(get_current_active_user)
):
    from fastapi.responses import StreamingResponse
    from io import BytesIO

    # Bulk fetch all selected POs
    pos = await mongo_db.purchase_orders.find(
        {"id": {"$in": po_ids}}, {"_id": 0}
    ).to_list(length=None)

    # Bulk fetch all related companies
    company_ids = list(
        set([po.get("company_id") for po in pos if po.get("company_id")])
    )
    companies = await mongo_db.companies.find(
...
async def create_inward_stock(
    inward_data: dict, current_user: dict = Depends(get_current_active_user)
):
    """
    ...
    if po_ids:
        for po_id in po_ids:
            ...
            for po_item in po.get("line_items", []):
                ...
                agg_key = (
                    sku
                    if (not product_id or str(product_id) in ("nan", "None", ""))
                    else product_id
                )
                    ...
                    async for existing_inward in mongo_db.inward_stock.find(
                        ...
                        for existing_item in existing_inward.get("line_items", []):
                            ...
                            if (
                                ...
                                product_id
                                and existing_item.get("product_id") == product_id
                            ):
                                matched = True
    ...
    inward_dict = {
        ...
        "pi_ids": inward_data.get(
            "pi_ids", [inward_data.get("pi_id")] if inward_data.get("pi_id") else []
        ),
...
async def create_pickup(
    pickup_data: dict, current_user: dict = Depends(get_current_active_user)
):
    """
    ...
    logger.info(
        f"🚚 Creating pickup. POs: {po_ids}, Warehouse: {pickup_data.get('warehouse_id')}"
    )
    ...
    for po_id in po_ids:
        ...
        for item in po.get("line_items", []):
            ...
            if key:
                aggregated_po_quantities[key] = aggregated_po_quantities.get(
                    key, 0
    ...
    for item in pickup_data.get("line_items", []):
        ...
        if not key:
            raise HTTPException(
                status_code=400,
                detail=f"Line item missing both product_id and sku for {item.get('product_name')}",
            )
        ...
        for inward in all_inwards:
            for line in inward.get("line_items", []):
                if (product_id and line.get("product_id") == product_id) or (
                    sku and line.get("sku") == sku
                ):
...
async def list_pickups(
    po_id: Optional[str] = None, current_user: dict = Depends(get_current_active_user)
):
    ...
    try:
        ...
        pickups = []
        async for pickup in mongo_db.pickup_in_transit.find(query, {"_id": 0}).sort(
            "created_at", -1
        ):
...
async def get_pickup(
    pickup_id: str, current_user: dict = Depends(get_current_active_user)
):
    """Get a single pickup entry"""
    pickup = await mongo_db.pickup_in_transit.find_one(
        {"id": pickup_id, "is_active": True}, {"_id": 0}
    )
    if not pickup:
...
async def update_pickup(
    ...
    await mongo_db.pickup_in_transit.update_one(
        {"id": pickup_id}, {"$set": update_data}
    )

    updated_pickup = await mongo_db.pickup_in_transit.find_one(
        {"id": pickup_id}, {"_id": 0}
    )
...
async def delete_pickup(
    ...
    await mongo_db.pickup_in_transit.update_one(
        {"id": pickup_id}, {"$set": {"is_active": False}}
    )
...
async def get_inward_stock(
    ...
    inward_entries = await mongo_db.inward_stock.find(query, {"_id": 0}).to_list(
        length=None
    )
    ...
    po_ids = list(
        set(entry.get("po_id") for entry in inward_entries if entry.get("po_id"))
    )
    company_ids = list(
        set(
    ...
    if po_ids:
        pos = await mongo_db.purchase_orders.find(
            {"id": {"$in": po_ids}},
            {"_id": 0, "id": 1, "voucher_no": 1, "company_id": 1},
        ).to_list(length=None)
    ...
    warehouses_map = {}
    if warehouse_ids:
        warehouses = await mongo_db.warehouses.find(
            {"id": {"$in": warehouse_ids}}, {"_id": 0}
        ).to_list(length=None)
...
async def get_direct_inward_entries(
    ...
    query = {"source_type": "direct_inward", "is_active": True}
    if warehouse_id:
                ...
                for out_item in outward.get("line_items", []):
                    if out_item.get("product_id") == item.get("product_id"):
                        dispatched_qty += float(
                            out_item.get("dispatch_quantity", 0)
                            or out_item.get("quantity", 0)
                        )
...
async def export_inward_stock(
    ...
    inward_entries = []
    async for entry in mongo_db.inward_stock.find(query, {"_id": 0}).sort(
        "created_at", -1
    ):
...
async def get_inward_stock_detail(
    inward_id: str, current_user: dict = Depends(get_current_active_user)
):
...
async def delete_inward_stock(
    ...
    has_dispatched = await mongo_db.stock_tracking.count_documents(
        {"inward_entry_id": inward_id, "quantity_outward": {"$gt": 0}}
    )
    ...
    delete_result = await mongo_db.stock_tracking.delete_many(
        {"inward_entry_id": inward_id}
    )
    if delete_result.deleted_count > 0:
        print(
            f"       ✅ Removed {delete_result.deleted_count} summary entries for inward: {inward_id}"
        )
    ...
    await mongo_db.inward_stock.update_one(
        {"id": inward_id}, {"$set": {"is_active": False}}
    )
...
async def update_stock_tracking(inward_entry: dict, operation: str):
    ...
    try:
        ...
        print(
            f"  🔄 Creating stock tracking entries for inward: {inward_entry.get('inward_invoice_no')}"
        )

        # Fetch warehouse details
        warehouse = await mongo_db.warehouses.find_one(
            {"id": inward_entry.get("warehouse_id")}, {"_id": 0, "name": 1}
        )
        ...
        if inward_entry.get("po_id"):
            po = await mongo_db.purchase_orders.find_one(
                {"id": inward_entry.get("po_id")}, {"_id": 0, "voucher_no": 1}
            )
        ...
        entry_type = (
            "direct"
            if inward_entry.get("source_type") == "direct_inward"
            else "regular"
        )
        ...
        for item in inward_entry.get("line_items", []):
            try:
                ...
                print(
                    f"     - Creating entry for: {item.get('product_name')} (Qty: {item.get('quantity')})"
                )

                # Create NEW stock entry for this transaction (NO aggregation)
                stock_entry = {
                    "id": str(uuid.uuid4()),
                    "inward_entry_id": inward_entry.get(
                        "id"
                    ),  # Link to source inward entry
                    "inward_invoice_no": inward_entry.get(
                        "inward_invoice_no"
                    ),  # For reference
                    ...
                    "quantity_inward": item[
                        "quantity"
                    ],  # Exact quantity from THIS entry only
                ...
                print(
                    f"       ✅ Created transaction entry: Inward {item['quantity']}, Remaining: {item['quantity']}"
                )
            except Exception as item_error:
                print(
                    f"       ❌ Error processing item {item.get('product_name')}: {str(item_error)}"
                )
                import traceback
...
async def get_stock_summary(
    ...
    it_pipeline = [
        ...
        {"$unwind": "$line_items"},
        {
            "$group": {
                "_id": "$line_items.sku",
                "total": {"$sum": "$line_items.quantity"},
            }
        },
    ]
    it_results = await mongo_db.pickup_in_transit.aggregate(it_pipeline).to_list(
        length=None
    )
                ...
                outward_entries.append(
                    {
                        ...
                        "quantity": item.get("dispatch_quantity")
...
async def delete_stock_summary(
    stock_id: str, current_user: dict = Depends(get_current_active_user)
):
    """
...
async def get_stock_transactions(
    ...
    async for inward in mongo_db.inward_stock.find(
        {**warehouse_query, "inward_type": "warehouse", "is_active": True}, {"_id": 0}
    ).sort("date", -1):
        for item in inward.get("line_items", []):
            if item.get("product_id") == product_id:
                transactions.append(
    ...
    for outward in all_outwards:
        ...
        if d_type == "dispatch_plan":
            type_label = "Dispatch Plan"
        elif d_type == "direct_export":
            type_label = "Direct Export"
...
async def create_outward_stock(
    outward_data: dict, current_user: dict = Depends(get_current_active_user)
):
    ...
    try:
        # Validate company
        company = await mongo_db.companies.find_one(
            {"id": outward_data.get("company_id")}, {"_id": 0}
        )
        if not company:
            log_this(
                f"  ❌ ERROR: Company not found - {outward_data.get('company_id')}"
            )
            raise HTTPException(status_code=404, detail="Company not found")
        ...
        outward_dict = {
            ...
            or f"EXP-{str(uuid.uuid4())[:8].upper()}",
        ...
        log_this(
            f"  📝 Processing {len(outward_data.get('line_items', []))} line items..."
        )
        ...
        for item in outward_data.get("line_items", []):
            ...
            # Stock Validation
            should_validate = outward_data.get("dispatch_type") == "dispatch_plan" or (
                outward_data.get("dispatch_type") == "export_invoice"
                and not outward_data.get("dispatch_plan_id")
            )
...
async def get_outward_stock(
    ...
    outward_entries = await mongo_db.outward_stock.find(query, {"_id": 0}).to_list(
        length=None
    )
    ...
    pis_map = {}
    if all_pi_ids:
        pis = await mongo_db.proforma_invoices.find(
            {"id": {"$in": all_pi_ids}}, {"_id": 0}
        ).to_list(length=None)
...
async def get_pending_dispatch_plans(
    ...
    async for dispatch in mongo_db.outward_stock.find(
        {"dispatch_type": "dispatch_plan", "is_active": True}, {"_id": 0}
    ):
...
async def export_outward_stock(
    ...
    outward_entries = []
    async for entry in mongo_db.outward_stock.find(
...
async def get_outward_stock_detail(
    outward_id: str, current_user: dict = Depends(get_current_active_user)
):
...
async def update_outward_stock(
    ...
    if old_entry.get("dispatch_type") == "export_invoice" and old_entry.get(
        "dispatch_plan_id"
    ):
    ...
    tracking_enabled = (
        old_entry.get("dispatch_type") == "dispatch_plan"
        or old_entry.get("dispatch_type") == "direct_export"
        or (
            old_entry.get("dispatch_type") == "export_invoice"
            and not old_entry.get("dispatch_plan_id")
        )
    )
    ...
    update_data = {
        "export_invoice_no": outward_data.get(
            "export_invoice_no", old_entry.get("export_invoice_no")
        ),
        "export_invoice_number": outward_data.get(
            "export_invoice_number", old_entry.get("export_invoice_number", "")
        ),
        ...
        "dispatch_mode": outward_data.get(
            "dispatch_mode", old_entry.get("dispatch_mode", "Export")
        ),
    ...
    updated_entry = await mongo_db.outward_stock.find_one(
        {"id": outward_id}, {"_id": 0}
    )
...
async def delete_outward_stock(
    ...
    await mongo_db.outward_stock.update_one(
        {"id": outward_id}, {"$set": {"is_active": False}}
    )
...
async def revert_stock_tracking_outward(outward_entry: dict):
    ...
    try:
        print(
            f"  🔄 Reverting stock tracking for outward: {outward_entry.get('export_invoice_no')}"
        )

        for item in outward_entry.get("line_items", []):
            try:
                # Support both quantity and dispatch_quantity fields
                qty_to_restore = item.get("dispatch_quantity") or item.get(
                    "quantity", 0
                )
                product_id = item.get("product_id")
                product_name = item.get("product_name")
                ...
                if product_id:
                    ...
                    print(
                        f"       ℹ️ No product_id, falling back to flexible SKU for restoration: {sku_val}"
                    )
                    tracking_query["sku"] = {
                        "$regex": f"^\\s*{sku_esc}",
                        "$options": "i",
                    ...
                    print(
                        f"       ❌ ERROR: Both product_id and SKU are missing for {product_name} in restoration"
                    )
                    continue
                ...
                for stock in stock_entries:
                    ...
                    qty_to_restore_here = min(
                        available_to_restore, remaining_to_restore
                    )
                    ...
                    print(
                        f"       ✅ Restored to entry (Invoice: {stock.get('inward_invoice_no')}): Outward {old_outward} → {new_outward}, Remaining {old_remaining} → {new_remaining}"
                    )
                ...
                if remaining_to_restore > 0:
                    print(
                        f"       ⚠️  Could not fully restore {remaining_to_restore} units of {product_name} (Stock mismatch?)"
                    )

            except Exception as item_error:
                print(
                    f"       ❌ Error restoring item {item.get('product_name')}: {str(item_error)}"
                )
                import traceback
...
async def get_available_stock(
    product_id: str, warehouse_id: str, sku: Optional[str] = None
) -> float:
...
async def update_stock_tracking_outward(outward_entry: dict):
    ...
    try:
        log_to_file(
            f"  🔄 Updating stock tracking for outward: {outward_entry.get('export_invoice_no')}"
        )
        log_to_file(f"     Warehouse ID: {outward_entry.get('warehouse_id')}")
        log_to_file(
            f"     Number of line items: {len(outward_entry.get('line_items', []))}"
        )
        ...
        for item in outward_entry.get("line_items", []):
            try:
                ...
                log_to_file(
                    f"     - Processing: {product_name} (Product ID: {product_id}, Dispatch Qty: {qty_to_dispatch})"
                )
                ...
                if sku_val:
                    ...
                    sku_esc = re.escape(sku_val)
                    log_to_file(
                        f"       ℹ️ Using flexible SKU matching for {product_name}: {sku_val}"
                    )
                    or_filters.append(
                        {"sku": {"$regex": f"^\\s*{sku_esc}", "$options": "i"}}
                    )

                if not or_filters:
                    log_to_file(
                        f"       ❌ ERROR: Both product_id and SKU are missing for {product_name}"
                    )
                    continue
                ...
                log_to_file(
                    f"       📦 Found {len(stock_entries)} stock entries with available stock"
                )

                if not stock_entries:
                    log_to_file(
                        f"       ⚠️  No stock available for {product_name} (ID: {product_id}) in warehouse {outward_entry.get('warehouse_id')}"
                    )
                    ...
                    log_to_file(
                        f"       📊 Total stock entries for this product: {total_entries}"
                    )
                    continue
                ...
                for stock in stock_entries:
                    ...
                    await mongo_db.stock_tracking.update_one(
                        {"id": stock.get("id")},
                        {
                            "$set": {
                                ...
                                "last_outward_date": datetime.now(
                                    timezone.utc
                                ).isoformat(),
                                "last_updated": datetime.now(timezone.utc).isoformat(),
                            }
                    ...
                    log_to_file(
                        f"       ✅ Updated entry (Invoice: {stock.get('inward_invoice_no')}): Outward {old_outward} → {new_outward}, Remaining: {new_remaining}"
                    )
                ...
                if remaining_to_dispatch > 0:
                    log_to_file(
                        f"       ⚠️  Insufficient stock! Could not dispatch {remaining_to_dispatch} units of {product_name}"
                    )
...
async def get_payment(
    payment_id: str, current_user: dict = Depends(get_current_active_user)
):
    """Get specific payment record with full details"""
    payment = await mongo_db.payments.find_one(
        {"id": payment_id, "is_active": True}, {"_id": 0}
    )
    if not payment:
    ...
    if payment.get("pi_id"):
        ...
        if pi:
            ...
            payment["calculated_pending_qty"] = (
                payment.get("total_quantity", 0) - dispatch_qty
            )
...
async def create_payment(
    payment_data: dict, current_user: dict = Depends(get_current_active_user)
):
    """Create new payment record for a PI"""
    # Validate PI exists
    pi = await mongo_db.proforma_invoices.find_one(
        {"id": payment_data["pi_id"]}, {"_id": 0}
    )
    if not pi:
        raise HTTPException(status_code=404, detail="PI not found")

    # Check if payment record already exists for this PI
    existing = await mongo_db.payments.find_one(
        {"pi_id": payment_data["pi_id"], "is_active": True}, {"_id": 0}
    )
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Payment record already exists for this PI. Please use edit to update.",
        )
    ...
    payment_dict = {
        ...
        "dispatch_goods_value": payment_data.get(
            "dispatch_goods_value", dispatch_value
        ),
...
async def update_payment(
    ...
    advance_payment = payment_data.get(
        "advance_payment", existing.get("advance_payment", 0)
    )
    received_amount = payment_data.get(
    ...
    update_data = {
        ...
        "dispatch_date": payment_data.get(
            "dispatch_date", existing.get("dispatch_date")
        ),
        "export_invoice_no": payment_data.get(
            "export_invoice_no", existing.get("export_invoice_no")
        ),
        "dispatch_goods_value": payment_data.get(
            "dispatch_goods_value", existing.get("dispatch_goods_value")
        ),
        "notes": payment_data.get("notes", existing.get("notes")),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }

    await mongo_db.payments.update_one({"id": payment_id}, {"$set": update_data})
...
async def add_payment_entry(
    ...
    payment_entries_total = sum(
        e.get("received_amount", 0) for e in updated_payment.get("payment_entries", [])
    )
...
async def get_export_details(
    ...
    async for outward in mongo_db.outward_stock.find(
        ...
        exported_qty = sum(
            item.get("dispatch_quantity", item.get("quantity", 0))
            for item in outward.get("line_items", [])
        )
        ...
        export_details.append(
            {
                ...
                "remaining_for_export": (
                    pi_total_qty - exported_qty if len(export_details) == 0 else 0
                ),
...
async def mark_short_payment(
    ...
    if not short_payment_data.get("note"):
        raise HTTPException(
            status_code=400, detail="Note is required for short payment"
        )
...
async def reopen_short_payment(
    ...
    if not payment.get("short_payment_status"):
        raise HTTPException(
            status_code=400, detail="Payment is not marked as short payment"
        )
...
async def get_extra_payments(
    pi_number: str = Query(...), current_user: dict = Depends(get_current_active_user)
):
    """Get all extra payments for a specific PI number"""
    extra_payments = []
    async for payment in mongo_db.pi_extra_payments.find(
        {"pi_number": pi_number, "is_active": True}, {"_id": 0}
    ).sort("date", -1):
...
async def create_extra_payment(
    ...
    try:
        ...
        # Verify bank exists
        bank = await mongo_db.banks.find_one(
            {"id": payment_data["bank_id"], "is_active": True}
        )
        if not bank:
            raise HTTPException(status_code=404, detail="Bank not found")
...
async def update_extra_payment(
    ...
    try:
        ...
        existing = await mongo_db.pi_extra_payments.find_one(
            {"id": extra_payment_id, "pi_number": pi_number, "is_active": True}
        )
        ...
        await mongo_db.pi_extra_payments.update_one(
            {"id": extra_payment_id}, {"$set": update_data}
        )
        ...
        updated = await mongo_db.pi_extra_payments.find_one(
            {"id": extra_payment_id}, {"_id": 0}
        )
        return updated
...
async def update_payment_with_extra_payments(pi_number: str):
    ...
    try:
        ...
        payment = await mongo_db.payments.find_one(
            {"pi_voucher_no": pi_number, "is_active": True}
        )
        ...
        payment_entries_total = sum(
            float(e.get("received_amount") or 0)
            for e in payment.get("payment_entries") or []
        )
    ...
    async for expense in mongo_db.expenses.find(query, {"_id": 0}).sort("date", -1):
        ...
        if expense.get("export_invoice_ids"):
            for inv_id in expense["export_invoice_ids"]:
                outward = await mongo_db.outward_stock.find_one(
                    {"id": inv_id, "is_active": True}, {"_id": 0}
                )
                if outward:
...
async def get_expense(
    expense_id: str, current_user: dict = Depends(get_current_active_user)
):
    """Get specific expense record with full details"""
    expense = await mongo_db.expenses.find_one(
        {"id": expense_id, "is_active": True}, {"_id": 0}
    )
    if not expense:
    ...
    if expense.get("export_invoice_ids"):
        for inv_id in expense["export_invoice_ids"]:
            ...
            if outward:
                ...
                items_value = sum(
                    item.get("amount", 0) for item in outward.get("line_items", [])
                )
...
async def create_expense(
    expense_data: dict, current_user: dict = Depends(get_current_active_user)
):
    ...
    if expense_data.get("export_invoice_ids"):
        for inv_id in expense_data["export_invoice_ids"]:
            ...
            if not outward:
                raise HTTPException(
                    status_code=404, detail=f"Export Invoice {inv_id} not found"
                )
    ...
    expense_dict = {
        "id": str(uuid.uuid4()),
        "expense_reference_no": expense_data.get("expense_reference_no")
        or f"EXP-{str(uuid.uuid4())[:8].upper()}",
...
async def update_expense(
    ...
    existing = await mongo_db.expenses.find_one(
        {"id": expense_id, "is_active": True}, {"_id": 0}
    )
    if not existing:
        raise HTTPException(status_code=404, detail="Expense record not found")
    ...
    freight_charges = expense_data.get(
        "freight_charges", existing.get("freight_charges", 0)
    )
    ...
    update_data = {
        ...
        "export_invoice_ids": expense_data.get(
            "export_invoice_ids", existing.get("export_invoice_ids")
        ),
        "export_invoice_nos_manual": expense_data.get(
            "export_invoice_nos_manual", existing.get("export_invoice_nos_manual")
        ),
        "freight_charges": freight_charges,
        "freight_vendor": expense_data.get(
            "freight_vendor", existing.get("freight_vendor")
        ),
        "cha_charges": cha_charges,
        "cha_vendor": expense_data.get("cha_vendor", existing.get("cha_vendor")),
        "other_charges": other_charges,
        "other_charges_description": expense_data.get(
            "other_charges_description", existing.get("other_charges_description")
        ),
        "total_expense": total_expense,
        "payment_status": expense_data.get(
    ...
    await mongo_db.expenses.update_one({"id": expense_id}, {"$set": update_data})
...
async def calculate_pl_report(
    request_data: dict, current_user: dict = Depends(get_current_active_user)
):
    """
    ...
    outwards = await mongo_db.outward_stock.find(
        {"id": {"$in": export_invoice_ids}, "is_active": True}, {"_id": 0}
    ).to_list(length=None)
    if not outwards:
    ...
    for o in outwards:
        pi_ids.extend(
            o.get("pi_ids", []) or ([o.get("pi_id")] if o.get("pi_id") else [])
        )
    ...
    po_query = {
        ...
        "$or": [
            ...
            },  # In case line items have sku at top level or we search by sku
        ],
    }
    # More robust: just fetch POs that might be relevant
    relevant_pos = await mongo_db.purchase_orders.find(po_query, {"_id": 0}).to_list(
        length=None
    )

    for po in relevant_pos:
        # Link to PIs
        p_ids = po.get("reference_pi_ids", []) or (
    ...
    expenses_list = await mongo_db.expenses.find(expense_query, {"_id": 0}).to_list(
        length=None
    )
    ...
    for outward in outwards:
        ...
        inv_pi_ids = outward.get("pi_ids", []) or (
            [outward.get("pi_id")] if outward.get("pi_id") else []
        )
        ...
        for item in outward.get("line_items", []):
            ...
            item_breakdown.append(
                {**item_data, "export_invoice_no": outward.get("export_invoice_no")}
            )
...
async def get_pi_po_mapping_list(
    ...
    pis = (
        await mongo_db.proforma_invoices.find(pi_query, {"_id": 0})
        .sort("date", -1)
        .skip(skip)
        .limit(page_size)
        .to_list(length=None)
    ...
    all_linked_pos = await mongo_db.purchase_orders.find(po_query, {"_id": 0}).to_list(
        length=None
    )
    ...
    for po in all_linked_pos:
        ref_ids = po.get("reference_pi_ids", []) or (
            [po.get("reference_pi_id")] if po.get("reference_pi_id") else []
        )
        for rid in ref_ids:
            if rid in pi_to_pos:
    ...
    for pi in pis:
        ...
        for po in pi_linked_pos:
            ...
            for po_item in po.get("line_items", []):
                ...
                pi_item = next(
                    (item for item in pi_items if item["sku"] == po_sku), None
                )
                if pi_item:
                    po_items.append(
                        {
                            ...
                            "remaining_quantity": pi_item["pi_quantity"]
        ...
        if sku:
            ...
            if not has_sku:
                has_sku = any(
                    any(sku.lower() in item["sku"].lower() for item in po["items"])
...
async def get_pi_po_mapping_detail(
    mapping_id: str, current_user: dict = Depends(get_current_active_user)
):
    """
    ...
    # Find the PI
    pi = await mongo_db.proforma_invoices.find_one(
        {"id": mapping_id, "is_active": True}, {"_id": 0}
    )
    ...
    # Calculate PI total quantity
    pi_total_quantity = sum(
        item.get("quantity", 0) for item in pi.get("line_items", [])
    )
...
async def get_inward_quantity(
    ...
    async for pi in mongo_db.proforma_invoices.find(pi_query, {"_id": 0}):
        ...
        po_query = {
            "$or": [{"reference_pi_id": pi["id"]}, {"reference_pi_ids": pi["id"]}],
            "is_active": True,
        }
        if po_number:
            po_query["voucher_no"] = {"$regex": po_number, "$options": "i"}
        ...
        async for po in mongo_db.purchase_orders.find(po_query, {"_id": 0}):
            ...
            for inward in inward_entries:
                for item in inward.get("line_items", []):
                    ...
                    if sku_key in pi_sku_quantities:
                        pi_sku_quantities[sku_key]["inward_quantity"] += item.get(
                            "quantity", 0
                        )
            ...
            total_inward_qty = sum(
                d["inward_quantity"] for d in pi_sku_quantities.values()
            )
...
async def get_outward_quantity(
    ...
    async for pi in mongo_db.proforma_invoices.find(pi_query, {"_id": 0}):
        ...
        all_outwards = await mongo_db.outward_stock.find(
            outward_query, {"_id": 0}
        ).to_list(None)

        # Deduplication: Track dispatch plans that are already converted to invoices
        invoiced_plan_ids = {
            o.get("dispatch_plan_id")
            for o in all_outwards
            if o.get("dispatch_type") == "export_invoice" and o.get("dispatch_plan_id")
        }
        ...
        for outward in all_outwards:
            ...
            if (
                outward.get("dispatch_type") == "dispatch_plan"
                and outward.get("id") in invoiced_plan_ids
            ):
                continue
            ...
            for item in outward.get("line_items", []):
                ...
                if match_key not in pi_sku_quantities:
                    ...
                    for k, d in pi_sku_quantities.items():
                        if (o_pid and d["product_id"] == o_pid) or (
                            o_sku and d["sku"] == o_sku
                        ):
                            found_key = k
                            break
                    match_key = found_key
                ...
                if match_key and match_key in pi_sku_quantities:
                    qty = float(
                        item.get("dispatch_quantity") or item.get("quantity", 0)
                    )
        ...
        # Calculate status
        total_pi_qty = sum(d["pi_quantity"] for d in pi_sku_quantities.values())
        total_outward_qty = sum(
            d["outward_quantity"] for d in pi_sku_quantities.values()
        )
...
async def get_dashboard_stats(current_user: dict = Depends(get_current_active_user)):
    ...
    outward_pipeline = [
                ...
                "invoiced_plans": [
                    {
                        "$match": {
                            "dispatch_type": "export_invoice",
                            "dispatch_plan_id": {"$exists": True, "$ne": None},
                        }
                    },
                    {
                        "$group": {
                            "_id": None,
                            "ids": {"$addToSet": "$dispatch_plan_id"},
                        }
                    },
                ],
                "all_entries": [{"$match": {}}],
        ...
        {"$unwind": "$all_entries"},
        {
            "$project": {
                "entry": "$all_entries",
                "invoiced_plan_ids": {
                    "$ifNull": [{"$arrayElemAt": ["$invoiced_plans.ids", 0]}, []]
                },
            }
        },
        ...
        {"$unwind": "$entry.line_items"},
        {
            "$group": {
                ...
                "total_qty": {
                    "$sum": {
                        "$ifNull": [
                            "$entry.line_items.quantity",
                            "$entry.line_items.dispatch_quantity",
                        ]
                    }
                },
            }
    ...
    pending_pis = await mongo_db.proforma_invoices.count_documents(
        {"is_active": True, "status": {"$in": ["Pending", "Draft"]}}
    )
...
async def get_customer_tracking(
    ...
    async for pi in mongo_db.proforma_invoices.find(pi_query, {"_id": 0}).sort(
        "date", -1
    ):
        # Get customer/company details - try company_id first, fallback to buyer/consignee
        customer = None
        if pi.get("company_id"):
            customer = await mongo_db.companies.find_one(
                {"id": pi.get("company_id")}, {"_id": 0}
            )
        if not customer and pi.get("customer_id"):
            customer = await mongo_db.companies.find_one(
                {"id": pi.get("customer_id")}, {"_id": 0}
            )
        ...
        for pi_item in pi.get("line_items", []):
            ...
            invoiced_plan_ids = {
                o.get("dispatch_plan_id")
                for o in all_outwards
                if o.get("dispatch_type") == "export_invoice"
                and o.get("dispatch_plan_id")
            }
            ...
            for outward in all_outwards:
                ...
                for outward_item in outward.get("line_items", []):
                    ...
                    if matches:
                        qty = float(
                            outward_item.get("dispatch_quantity", 0)
                            or outward_item.get("quantity", 0)
                        )
            ...
            tracking_data.append(
                {
                    ...
                    "linked_po_numbers": [
                        po.get("voucher_no") or po.get("po_no") for po in linked_pos
                    ],
        ...
        for pi in pis:
            ...
            for pi_item in pi.get("line_items", []):
                ...
                for inward in inwards:
                    ...
                    is_linked = False
                    if inward.get("pi_id") == pi["id"] or pi["id"] in inward.get(
                        "pi_ids", []
                    ):
                        is_linked = True
                    elif inward.get("po_id") in po_ids or any(
                        pid in po_ids for pid in inward.get("po_ids", [])
                    ):
                        # Narrow down: must be a PO linked to THIS PI
                        inward_po_id = inward.get("po_id")
                        relevant_po = next(
                            (p for p in pos if p["id"] == inward_po_id), None
                        )
                        if relevant_po and (
                            relevant_po.get("reference_pi_id") == pi["id"]
                            or pi["id"] in relevant_po.get("reference_pi_ids", [])
                        ):
                            is_linked = True
                ...
                for outward in outwards:
                    is_linked = False
                    if outward.get("pi_id") == pi["id"] or pi["id"] in outward.get(
                        "pi_ids", []
                    ):
                        is_linked = True
                    elif outward.get("po_id") in po_ids or any(
                        pid in po_ids for pid in outward.get("po_ids", [])
                    ):
                        outward_po_id = outward.get("po_id")
                        relevant_po = next(
                            (p for p in pos if p["id"] == outward_po_id), None
                        )
                        if relevant_po and (
                            relevant_po.get("reference_pi_id") == pi["id"]
                            or pi["id"] in relevant_po.get("reference_pi_ids", [])
...
async def get_purchase_analysis(
    ...
    try:
        ...
        if not company_id_list or not pi_number_list:
            return {
                "message": "Please select Company and PI Number filters",
                "data": [],
            }
        ...
        inwards = await mongo_db.inward_stock.find(
            {"po_id": {"$in": po_ids}, "inward_type": "warehouse", "is_active": True},
            {"_id": 0},
        ).to_list(None)

        pickups = await mongo_db.pickup_in_transit.find(
            {"po_id": {"$in": po_ids}, "is_active": True, "is_inwarded": {"$ne": True}},
            {"_id": 0},
        ).to_list(None)
            ...
            for po_item in po.get("line_items", []):
                ...
                for pi in pis:
                    if pi.get("id") in ref_pi_ids:
                        ...
                        for pi_item in pi.get("line_items", []):
                            if (
                                product_id and pi_item.get("product_id") == product_id
                            ) or (sku and pi_item.get("sku") == sku):
                                pi_quantity = float(pi_item.get("quantity", 0))
                                break
                        break
                ...
                for inward in inwards:
                    if inward.get("po_id") == po_id:
                        for item in inward.get("line_items", []):
                            if (
                                (product_id and item.get("product_id") == product_id)
                                or (sku and item.get("sku") == sku)
                                or (
                                    item.get("id") == po_item.get("id")
                                    and po_item.get("id")
                                )
                            ):
                ...
                analysis_data.append(
                    {
                        ...
                        "remaining_quantity": po_quantity
                        - inward_quantity
                        - intransit_quantity,
                    }
                )
...
@api_router.get("/pos/lines-with-stats")
async def get_po_lines_with_stats(
    voucher_no: str, current_user: dict = Depends(get_current_active_user)
):
    """
    Get PO line items with detailed statistics:
    - PI Qty (from PI reference)
    ...
    try:
        ...
        if not voucher_nos:
            raise HTTPException(
                status_code=400, detail="At least one voucher number is required"
            )

        # 1. Find all POs by voucher numbers
        pos = await mongo_db.purchase_orders.find(
            {"voucher_no": {"$in": voucher_nos}, "is_active": True}, {"_id": 0}
        ).to_list(length=100)
        if not pos:
            raise HTTPException(
                status_code=404,
                detail=f"No active POs found for vouchers: {voucher_no}",
            )
        ...
        if all_pi_ids:
            pis = await mongo_db.proforma_invoices.find(
                {"id": {"$in": list(set(all_pi_ids))}, "is_active": True}, {"_id": 0}
            ).to_list(None)

        # 3. Bulk fetch ALL inward stock for these POs
        inwards = await mongo_db.inward_stock.find(
            {"po_id": {"$in": po_ids}, "is_active": True}, {"_id": 0}
        ).to_list(None)
        ...
        for po in pos:
            ...
            for po_item in po.get("line_items", []):
                ...
                for pi in pis:
                    for pi_item in pi.get("line_items", []):
                        if (sku and pi_item.get("sku") == sku) or (
                            not sku and pi_item.get("product_id") == product_id
                        ):
                ...
                for inward in inwards:
                    if inward.get("po_id") == po_id:
                        for inward_item in inward.get("line_items", []):
                            if inward_item.get("id") == po_item_id or (
                                not inward_item.get("id")
                                and sku
                                and inward_item.get("sku") == sku
                            ):
                                stats["already_inwarded"] += float(
                ...
                for pickup in pickups:
                    if pickup.get("po_id") == po_id:
                        for pickup_item in pickup.get("line_items", []):
                            if pickup_item.get("id") == po_item_id or (
                                not pickup_item.get("id")
                                and sku
                                and pickup_item.get("sku") == sku
                            ):
                                stats["in_transit"] += float(
        ...
        for key, stats in aggregated_stats.items():
            stats["available_for_pickup"] = (
                stats["po_quantity"] - stats["already_inwarded"] - stats["in_transit"]
            )
...
async def bulk_delete_inward_stock(
    ...
    for inward_id in ids:
        try:
            inward = await mongo_db.inward_stock.find_one(
                {"id": inward_id, "is_active": True}, {"_id": 0}
            )
            if not inward:
...
async def bulk_delete_outward_stock(
    ...
    for outward_id in ids:
        try:
            outward = await mongo_db.outward_stock.find_one(
                {"id": outward_id, "is_active": True}, {"_id": 0}
            )
            if not outward:
