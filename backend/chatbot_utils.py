...
async def tool_get_document_details(voucher_no: str):
    ...
    pi = await mongo_db.proforma_invoices.find_one(
        {"voucher_no": voucher_no, "is_active": True}, {"_id": 0}
    )
    if pi:
        ...
        return {
            ...
            "items": [
                {"sku": i.get("sku"), "qty": i.get("quantity"), "rate": i.get("rate")}
                for i in items
            ],
        }
...
async def tool_search_documents_by_company(company_name: str, doc_type: str = "PI"):
    ...
    if not docs:
        return {
            "message": f"No {doc_type}s found for company matching '{company_name}'"
        }
...
async def tool_get_stock_summary_by_category(category: str):
    ...
    products = await mongo_db.products.find(
        {"category": {"$regex": category, "$options": "i"}, "is_active": True}
    ).to_list(None)
    if not products:
...
async def tool_get_recent_transactions(limit: int = 5):
    ...
    inwards = (
        await mongo_db.inward_stock.find({"is_active": True}, {"_id": 0})
        .sort("date", -1)
        .limit(limit)
        .to_list(limit)
    )
    outwards = (
        await mongo_db.outward_stock.find({"is_active": True}, {"_id": 0})
        .sort("date", -1)
        .limit(limit)
        .to_list(limit)
    )
...
async def tool_get_pending_documents(doc_type: str = "PI"):
    ...
    cursor = (
        mongo_db[collection]
        .find({"status": "Pending", "is_active": True}, {"_id": 0})
        .sort("date", -1)
        .limit(10)
    )
...
async def chat_with_bora_assistant(message: str, history: list = None):
    ...
    chat = model.start_chat(
        history=history or [], enable_automatic_function_calling=False
    )
    ...
    try:
        ...
        if response.candidates[0].content.parts[0].function_call:
            ...
            if tool_name == "get_document_details":
                ...
                tool_result = await tool_search_documents_by_company(
                    args.get("company_name"), args.get("doc_type", "PI")
                )
            elif tool_name == "get_stock_summary_by_category":
                tool_result = await tool_get_stock_summary_by_category(
                    args.get("category")
                )
            elif tool_name == "get_recent_transactions":
                tool_result = await tool_get_recent_transactions(
                    int(args.get("limit", 5))
                )
            elif tool_name == "get_pending_documents":
                tool_result = await tool_get_pending_documents(
                    args.get("doc_type", "PI")
                )
            else:
            ...
            response = chat.send_message(genai.protos.Content(parts=[part]))
