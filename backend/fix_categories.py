...
async def fix_stock_tracking_categories():
    ...
    async for stock in cursor:
        ...
        if product_id:
            ...
            if product:
                category = (
                    product.get("category") or product.get("Category") or "Unknown"
                )
        ...
        if category == "Unknown" and stock.get("inward_entry_id"):
            inward = await db.inward_stock.find_one(
                {"id": stock.get("inward_entry_id")}
            )
            if inward:
        ...
        if category != "Unknown":
            await db.stock_tracking.update_one(
                {"_id": stock["_id"]}, {"$set": {"category": category}}
            )
            updated += 1
